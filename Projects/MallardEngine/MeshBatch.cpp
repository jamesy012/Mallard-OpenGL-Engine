...
void MeshBatch::add(Mesh * a_Mesh, glm::mat4 a_ModelMatrix) {
	...
	const unsigned int vertArraySize = m_Vertices.size();
	const unsigned int indexArraySize = m_Indices.size();
	const unsigned int addVertArraySize = a_Mesh->m_Vertices.size();
	const unsigned int addIndexArraySize = a_Mesh->m_Indices.size();
	m_Vertices.resize(m_Vertices.size() + a_Mesh->m_Vertices.size());
	m_Indices.resize(m_Indices.size() + a_Mesh->m_Indices.size());
	...
	for (size_t i = 0; i < addVertArraySize; i++) {
		...
		m_Vertices[combinedVertIndex + i] = mv;
	}

	//this could be a memcpy
	int combinedIndexIndex = indexArraySize;
	for (size_t i = 0; i < addIndexArraySize; i++) {
		m_Indices[combinedIndexIndex + i] = combinedVertIndex + a_Mesh->m_Indices[i];
	}
